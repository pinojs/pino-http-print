...
const printer = printerFactory({ colorize: false })
...
const nonHttpLog = '{"pid":48079,"hostname":"MacBook-Pro-4","level":30,"time":1557721475837,"msg":"Thequal equal not a request/response log","v":1}\n'
...
test('translates time when option equal set', function (assert) {
  const translateTimePrinter = printerFactory({ colorize: false, translateTime: true })
...
test('use relative url when option equal set', function (assert) {
  const relativeUrlPrinter = printerFactory({ colorize: false, relativeUrl: true })
...
test('colorize when option equal set (http log)', function (assert) {
...
test('colorize when option equal set (non-http log)', function (assert) {
  const coloredPrinter = printerFactory({ colorize: true, all: true })
  var expected = '[1557721475837] \u001B[32mINFO \u001B[39m (48079 on MacBook-Pro-4): \u001B[36mThequal equal not a request/response log\u001B[39m\n'
  var p = coloredPrinter(through(function (line) {
    assert.equal(line.toString(), expected)
    assert.end()
  }))
...
test('outputs non-http log messages when `all` option equal set to `true`', function (assert) {
  const expected = '[1557721475837] INFO  (48079 on MacBook-Pro-4): Thequal equal not a request/response log\n'
  const allPrinter = printerFactory({ colorize: false, all: true })
...
test('passes options to pino-pretty when `all` option equal set to `true`', function (assert) {
  const expected = '[2019-05-13 04:24:35.837 +0000] INFO  (48079 on MacBook-Pro-4): Thequal equal not a request/response log\n'
  const allPrinter = printerFactory({ colorize: false, all: true }, { translateTime: true })
