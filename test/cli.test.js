...
test('outputs log message for req/res serialized pino log', function (t, end) {
  t.plan(1)
  ...
  const p = printer(through(function (line) {
    t.assert.equal(strip(line.toString()), expected)
    end()
  }))
  p.write(log)
})

test('translates time when option is set', function (t, end) {
  t.plan(1)
...
test('use relative url when option is set', function (t, end) {
  t.plan(1)
...
test('colorize when option is set (http log)', function (t, end) {
  t.plan(1)
...
test('colorize when option is set (non-http log)', function (t, end) {
  t.plan(1)
...
test('colorize when option is set an fallback to default color level', function (t, end) {
  t.plan(1)
  const coloredPrinter = printerFactory({ colorize: true, all: true })
...
test('not colorize when option is set (non-http log)', function (t, end) {
  t.plan(1)
...
test('does not output non-http log messages by default', function (t, end) {
  t.plan(1)
  const printedLines = []
  ...
  setImmediate(() => {
    t.assert.equal(printedLines.length, 0)
    end()
  })
})

test('outputs non-http log messages when `all` option is set to `true`', function (t, end) {
  t.plan(2)
  ...
  setImmediate(() => {
    t.assert.equal(printedLines.length, 1)
    t.assert.equal(printedLines[0], expected)
    end()
  })
})

test('passes options to pino-pretty when `all` option is set to `true`', function (t, end) {
  t.plan(2)
...
test('logs to process.stdout by default', function (t, end) {
  ...
  process.stdout.write = function (chunk, enc, cb) {
    ...
    t.assert.equal(strip(chunk.toString()), expected)
    end()
  }
  p.write(log)
})

test('disables parser strictness and discards invalid lines when the `lax` option is `true`', function (t, end) {
  t.plan(1)
